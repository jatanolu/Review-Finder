...
function Profile(logged) {
	...
	<Form.Control type='text' placeholder='List Name' className='me-2'
		aria-label='usr' onChange={(event)=>settext(event.target.value)}/>
		<Button onClick={doThat} className='mx-3 px-5' variant='outline-success'>Create List</Button>

		</Form>
		<Button onClick={()=>{console.log(list)}}/>

		{list ? list.map((ele)=>{ return <div style={{top:500, right:300}}><p>{ele.fields.name}</p></div>}):null}
		</>
