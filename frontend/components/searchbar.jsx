...
function Searchbar(request) {
  ...
  mapboxgl.accessToken =
    "pk.eyJ1IjoiamF0YW5vIiwiYSI6ImNsYndxOWY1NjA2cWszdnA4YzA5N20xYmcifQ.1cY-iw6TUx16Oh0yjWFbMg";
  ...
  const searchThat = async () => {
    ...
    let myResponse = await axios.post("search/", {
      loc: request["searched"]["location"],
      searching: request["searched"]["searching"],
    });
            ...
            <Col md="auto">
              <Card bg="light" className="mx-4 text-center" style={{ width: "24rem" }}>
                <Card.Body>
                  ...
                  <Button
                    onClick={() => {
                      window.location.replace(`${business.url}`);
                    }}
                    variant="primary"
                  >
                    Go to Yelp site
        ...
        <div style={{ top: 56, right: 900, position: "fixed", height: "100vh", width: 400 }}>
          <Card bg="light" className="mx-4 text-center" style={{ width: "30rem" }}>
            <Card.Body>
              ...
              {review
                ? review.map((op) => {
                    return <Card.Text>Review: {op.text}</Card.Text>;
                  })
                : null}
              {request.logged ? (
